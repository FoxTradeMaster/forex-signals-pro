...
export async function getHistoricalPerformance(days = 30) {
  ...
  let result = await db
    .select({
      ...
      outcome: (signalPerformance as any).outcome,
      closedAt: (signalPerformance as any).closedAt,
  ...
  return {
    ...
    signals: result.map((s) => ({
      ...
      outcome: (s as any).outcome as string | null,
      closedAt: (s as any).closedAt as Date | null,
